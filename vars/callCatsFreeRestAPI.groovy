...
def call(String name = "unknown") {
  ...
  def srvResponse = sh(resturnStdout: true, script: "curl -X GET -H 'content-type: application/json' ${srvUrl} | jq -c '.' ")
  println "Response before processing it: ${srvResponse}"
  def response = sh(returnStdout: true, script: "echo ${srvResponse} | jq -c '.fact'").trim()
